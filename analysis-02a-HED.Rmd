...
(H1c): The effect of role overload on alcohol misuse is mediated by psychological distress _(see Model 3b and Model 4b)_
...
outfit_HED_1 %>%
  ...
  kable(booktabs = TRUE, align = c('c','c','c','c'),
        ...
        caption = paste("Outcome: RAPI; N = ", N_HED_1, sep=""), escape = FALSE) %>%
...
outfit_HED_2 %>%
  ...
  kable(booktabs = TRUE, align = c('c','c','c','c'),
        col.names = linebreak(c("Parameter","Estimates","SE","p-value"), align = "c"),
        caption = paste("Outcome: RAPI; N = ", N_HED_2, sep=""), escape = FALSE) %>%
  kable_styling(full_width = FALSE, latex_options = c("HOLD_position"))  
```
...
# Estimate Mediated Effect using Model 3b and Model 4b


```{r, echo = FALSE, message = FALSE, warning = FALSE}
# Read in bootstrap estimates
load("list_models_HED_02a.RData")
alpha <- 0.05
```
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
library(MuMIn)
all_qic <- QIC(fit_HED_1, fit_HED_2, fit_DASS_1)
