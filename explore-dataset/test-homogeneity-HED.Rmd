...
1. Participants who were **included** in analyses having HED as the outcome of interest 
1. Participants who were **excluded** in analyses having HED as the outcome of interest 
...
# Results for HED as outcome of interest
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
keep_rows <- complete.cases(dat_analysis[, !(colnames(dat_analysis) %in% c("baseline_rutgers", "rutgers"))])
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
dat_results_HED <- data.frame(variable = NULL, time = NULL, pval = NULL)
```
...
dat_results_HED <- rbind(dat_results_HED, dat_results_tmp)
...
dat_results_tmp <- data.frame(variable = "baseline RAPI", pval = res[["p.value"]])
dat_results_HED <- rbind(dat_results_HED, dat_results_tmp)
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
dat_results_HED[["pval"]] <- format(round(dat_results_HED[["pval"]], 3), nsmall = 3)
```

```{r, echo = FALSE, message = FALSE, warning = FALSE}
colnames(dat_results_HED) <- c("Variable", "p")

kable(dat_results_HED, 
      booktabs=TRUE, 
...
df_merged_baseline_drinking[nrow(df_merged_baseline_drinking),1] <- "Average value of baseline RAPI"

kable(df_merged_baseline_drinking, 
      booktabs=TRUE, 
      caption = "Frequency table for baseline RAPI (among those participants in each sample whose baseline RAPI is not missing)", 
      escape = FALSE) %>%
  kable_styling(position = "center", latex_options = "HOLD_position")
```
