...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
dat_subset <- dat_screener_analysis[, c("baseline_rutgers","baseline_condition")]
dat_subset <- dat_subset %>% filter(baseline_condition!=1)

true_nrows <- nrow(dat_subset)
true_missing <- sum(is.na(dat_subset$baseline_rutgers))
