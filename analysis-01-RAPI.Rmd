...
outfit_rutgers_8 %>%
  ...
  kable(booktabs = TRUE, align = c('c','c','c','c'),
        col.names = linebreak(c("Parameter","Estimates","SE","p-value"), align = "c"),
        caption = paste("Outcome: Rutgers Alcohol Problem Index; N = ", N_rutgers_8, sep=""), escape = FALSE) %>%
  kable_styling(full_width = FALSE, latex_options = c("HOLD_position"))  
```
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
library(MuMIn)
all_qic <- QIC(fit_rutgers_1, fit_rutgers_2, fit_rutgers_3, fit_rutgers_4, fit_rutgers_5, fit_rutgers_6, fit_rutgers_7, fit_rutgers_8)
all_qic <- data.frame(Table = c(2,3,4,5,6,7,12,13), QIC = all_qic$QIC)

all_qic %>%
  kable(booktabs = TRUE, align = c('c','c'),
        col.names = c("Table No.", "QIC"),
        caption = paste("Outcome: Rutgers Alcohol Problem Index", sep=""), escape = FALSE) %>%
  kable_styling(full_width = FALSE, latex_options = c("HOLD_position"))  
```
