...
dat_corr <- cbind(dat_baseline_analysis[,dat_baseline_vars$new_var_name],
                  dat_screener_analysis[,dat_screener_vars$new_var_name],
                  dat_wnw_0,
                  dat_wnw_1)
cormat <- cor(dat_corr, use = "pairwise.complete.obs")
cormat <- round(cormat, digits=3)

write.csv(cormat, "correlation_matrix.csv", row.names = TRUE)
```
