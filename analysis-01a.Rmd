...
outfit_HED_3 %>%
  ...
  kable(booktabs = TRUE, align = c('c','c','c','c'),
        col.names = linebreak(c("Parameter","Estimates","SE","p-value"), align = "c"),
        caption = paste("Outcome: No. of Days in the Past Month with Heavy Episodic Drinking; N = ", N_HED_3, sep=""), escape = FALSE) %>%
  kable_styling(full_width = FALSE, latex_options = c("HOLD_position"))  

ss_tab %>%
  mutate(parameter = as.character(row.names(.))) %>%
  select(parameter, everything()) %>%
  kable(booktabs = TRUE, align = c('c','c','c','c','c','c','c'), caption = paste("Outcome: No. of Days in the Past Month with Heavy Episodic Drinking; N = ", N_HED_3, sep="")) %>%
  kable_styling(full_width = FALSE, latex_options = c("HOLD_position", "scale_down")) 
```
