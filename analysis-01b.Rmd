...
(H1b): Role overload is associated with an increase in alcohol misuse among young adults transitioning from college to work.
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
fit_rutgers_3 <- geem(rutgers ~ sex + race + age + baseline_rutgers + baseline_social_desirability + baseline_impulsivity + lifestress 
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
fit_HED_3 <- geem(HED ~ sex + race + age + baseline_HED + baseline_social_desirability + baseline_impulsivity + lifestress 
                      + descriptive_workplace_norms
                      + qualitative_role_overload + quantitative_role_overload, 
                  data = dat_analysis, id = ParticipantID, waves = time, corstr = "exchangeable", family = "quasipoisson")
