...
(H2): The distress-mediated effect of role overload on alcohol misuse weakens over time: It is stronger during the early onboarding phase than the late onboarding phase _(see Model 5a and Model 6a)_

(H3): During the early onboarding phase, the distress-mediated effect of role overload on alcohol misuse is curvilinear; it diminishes to the extent that psychological distress increases _(see Model 7a and Model 6a)_
...
# Estimate Mediated Effect using Model 5a and Model 6a


```{r, echo = FALSE, message = FALSE, warning = FALSE}
# Read in bootstrap estimates
load("list_models_rutgers_02b.RData")
alpha <- 0.05
```
...
# Estimate Mediated Effect using Model 7a and Model 6a


```{r, echo = FALSE, message = FALSE, warning = FALSE}
# Read in bootstrap estimates
load("list_models_rutgers_02c.RData")
alpha <- 0.05
```
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
# Read in bootstrap estimates
load("list_models_rutgers_02d.RData")
alpha <- 0.05
```
...
outfit_rutgers_5 %>%
  ...
  kable(booktabs = TRUE, align = c('c','c','c','c'),
        col.names = linebreak(c("Parameter","Estimates","SE","p-value"), align = "c"),
        caption = paste("Outcome: RAPI; N = ", N_rutgers_5,  "; QIC = ", round(QIC(fit_rutgers_5), digits = 2), sep=""), escape = FALSE) %>%
  kable_styling(full_width = FALSE, latex_options = c("HOLD_position"))  
```
...
outfit_rutgers_6 %>%
  ...
  kable(booktabs = TRUE, align = c('c','c','c','c'),
        col.names = linebreak(c("Parameter","Estimates","SE","p-value"), align = "c"),
        caption = paste("Outcome: RAPI; N = ", N_rutgers_6,  "; QIC = ", round(QIC(fit_rutgers_6), digits = 2), sep=""), escape = FALSE) %>%
  kable_styling(full_width = FALSE, latex_options = c("HOLD_position"))  
```


## Estimate Mediated Effect using Model 8a and Model 6a
...
for(i in 1:length(list_models_rutgers_02d)){
  curr_model_estimates <- list_models_rutgers_02d[[i]]
  ...
  curr_params_rutgers <- data.frame(a2=a2, a4=a4, b1=b1, b2=b2)
  list_params_rutgers <- append(list_params_rutgers, list(curr_params_rutgers))
}
```
...
bootdat_params_rutgers <- bind_rows(list_params_rutgers) %>%
  ...
  mutate(qualitative_role_overload_time_0_DASS_low = a2*(b1-b2),
         qualitative_role_overload_time_0_DASS_moderate = a2*(b1),
         qualitative_role_overload_time_0_DASS_high = a2*(b1+b2),
         qualitative_role_overload_time_1_DASS_low = (a2+a4)*(b1-b2),
         qualitative_role_overload_time_1_DASS_moderate = (a2+a4)*(b1),
         qualitative_role_overload_time_1_DASS_high = (a2+a4)*(b1+b2))
```
