...
outfit_rutgers_4 %>%
  ...
  kable(booktabs = TRUE, align = c('c','c','c','c'),
        col.names = linebreak(c("Parameter","Estimates","SE","p-value"), align = "c"),
        caption = paste("Outcome: RAPI; N = ", N_rutgers_4, sep=""), escape = FALSE) %>%
  kable_styling(full_width = FALSE, latex_options = c("HOLD_position"))  
```
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
library(MuMIn)
all_qic <- QIC(fit_rutgers_1, fit_rutgers_2, fit_DASS_1, fit_rutgers_3, fit_rutgers_4)
all_qic <- data.frame(Table = c(1,2,3,4,5), QIC = all_qic$QIC)

all_qic %>%
  kable(booktabs = TRUE, align = c('c','c'),
        col.names = c("Table No.", "QIC"),
        caption = paste("Outcome: RAPI", sep=""), escape = FALSE) %>%
  kable_styling(full_width = FALSE, latex_options = c("HOLD_position"))  
```
